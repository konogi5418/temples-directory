...
export default function AdminDashboard() {
  ...
  const handleSaveLastUpdated = async () => {
    try {
      ...
      alert('名簿の更新日を保存しました。');
    } catch (err: any) { alert(`保存エラー: ${err.message}`); }
  };
  ...
  const handleExportCSV = () => { 
    ...
    const csvRows = temples.map(t => { 
      ...
      return [ 
        ...
        `"${(t.priest_name || '').replace(/"/g, '""')}"`, `"${(t.acting_priest || '').replace(/"/g, '""')}"`, 
        `"${(t.vice_priest || '').replace(/"/g, '""')}"`, `"${(t.resident_priests || '').replace(/"/g, '""')}"`, 
        t.is_church ? 'TRUE' : 'FALSE', `"${branchesStr.replace(/"/g, '""')}"` 
      ].join(','); 
    }); 
    const csvContent = '\uFEFF' + headers.join(',') + '\n' + csvRows.join('\n'); 
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); 
    const url = URL.createObjectURL(blob); 
    const link = document.createElement('a'); link.href = url; link.setAttribute('download', 'temples_backup.csv'); document.body.appendChild(link); link.click(); document.body.removeChild(link); 
  };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', borderBottom: '2px solid #ccc', paddingBottom: '10px' }}>
        <button onClick={() => setActiveTab('temples')} style={{ padding: '10px 20px', fontSize: '16px', cursor: 'pointer', background: activeTab === 'temples' ? '#0056b3' : '#f8f9fa', color: activeTab === 'temples' ? 'white' : 'black', border: '1px solid #ccc', borderRadius: '4px' }}>寺院データ管理</button>
        <button onClick={() => setActiveTab('departments')} style={{ padding: '10px 20px', fontSize: '16px', cursor: 'pointer', background: activeTab === 'departments' ? '#0056b3' : '#f8f9fa', color: activeTab === 'departments' ? 'white' : 'black', border: '1px solid #ccc', borderRadius: '4px' }}>総本山 部署データ管理</button>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ccc' }}>
            <thead>
              <tr style={{ background: '#f5f5f5', textAlign: 'left' }}>
                ...
                <th style={{ padding: '10px', borderBottom: '1px solid #ccc', width: '30%' }}>役職者一覧</th>
                  ...
                  <td style={{ padding: '10px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        {editingId === temple.id ? (
                          <input type="text" value={editPriestName} onChange={(e) => setEditPriestName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && savePriestName(temple.id)} onBlur={() => savePriestName(temple.id)} autoFocus style={{ padding: '2px', width: '100px' }} />
                        ) : (
                      ...
                      {temple.resident_priests && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <span style={{ fontSize: '13px', color: '#444' }}>{temple.resident_priests}</span>
                        </div>
                      )}
                    </div>
                  </td>
